...
def parameters_count(temperature, material, concentration, activate_energy, acceptor):
    ...
    n=decimal.Decimal(n)
    ni=decimal.Decimal(ni)
    n=str(format(n, '.2e'))

    ni=str(format(ni, '.2e'))

    return round(F, 5), n, round(lnn, 3), ni, round(reverse_temp, 3), round(Eg, 3), round(Fi, 3), round(mu, 3)
    print("Function for count parameters")
if __name__ == '__main__':
    main()
# Change temperatures function also

# parameters('GaAs', 2e16, 0.031, 300, False)
# parameters('GaAs', 2e15, 0.031, 300, False)
# parameters('Ge', 4e16, 0.011, 300, True)
# parameters('Si', 1e17, 0.045, 300, True)
